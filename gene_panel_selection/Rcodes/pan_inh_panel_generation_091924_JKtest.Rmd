...
# JK - selecting seeds. Default seed for subsampleCells is 5
seed = 5

sub <- 50                                                                       # Number of cells to subsample per cluster
# sampled_cells   <- subsampleCells(anno$cluster_label,sub)                       # Subsample cells
sampled_cells   <- subsampleCells(anno$cluster_label,sub, seed=seed)                       # Subsample cells - JK, to add seed
...
if (length(built_panel)<num_good_genes){
  good_genes <- built_panel[1:num_good_genes]
  bad_genes <- setdiff(built_panel,good_genes)
}else{
  good_genes <- built_panel
...
## Subsample cells from each cluster which decreases the time for the algorithm to run and also more evenly weights the clusters for gene selection
# JK to test random seed. Default random seed in subsampleCells is 5
seed = 20  # JK: change this seed to test different results

sub <- 50                                                                       # Number of cells to subsample per cluster
# sampled_cells   <- subsampleCells(anno$cluster_label,sub)                       # Subsample cells
sampled_cells   <- subsampleCells(anno$cluster_label,sub, seed=seed)            # Subsample cells (JK)
...
calculate_cells <- is.element(anno$cluster_label,keep_clusts)
...
full_panel_correct <- fractionCorrectWithGenes(full_built_panel,norm_exp_data[,calculate_cells],median_expr[,keep_clusts],cl[calculate_cells],plot=FALSE)
full_panel_scaled <- t(t(full_panel_correct)/max(full_panel_correct))

gene_panel <- full_built_panel

# JK save the results, combining gene_panel and full_panel_correct, after making them as dataframes
selection_results <- data.frame(Gene = gene_panel, Accuracy = full_panel_correct)
...
write.csv(selection_results, file = paste0(output_folder, "/gene_panel_selection_results_allcellclassification_seed", seed, ".csv"), row.names = FALSE)
...
confusion_matrix <- pl
View(conf_matrix %>%filter(realCluster==foundCluster))
...
for (cl in c(1:length(keep_clusts))){
  anno$cluster_id[anno$cluster_label == keep_clusts[cl]]<-cl
}
